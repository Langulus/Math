...
namespace Langulus::Math
{
	...
	TEMPLATE()
	template<DenseNumber ALTT, pcptr ALTC, pcptr ALTR>
	constexpr TME()::TMatrix(const TMatrix<ALTT, ALTC, ALTR>& a) noexcept {
		if constexpr (ALTC != COLS || ALTR != ROWS) {
			if constexpr (ALTC < COLS || ALTR < ROWS) {
				// If copied region is smaller, make sure to reset				
				*this = Identity();
			}

			for (pcptr col = 0; col < pcMin(COLS, ALTC); ++col) {
				for (pcptr row = 0; row < pcMin(ROWS, ALTR); ++row) {
					if constexpr (Same<ALTT, T>) {
						// Direct copy														
			...
			pcCopyMemory(a.mArray, mArray, sizeof(T) * MemberCount);
		}
	}

	/// Scalar constructor - sets all elements											
	///	@param x - scalar value to spread across the whole matrix				
	TEMPLATE()
	template<Number K>
	constexpr TME()::TMatrix(K x) noexcept {
		if constexpr (Dense<K>) {
			const T e = static_cast<T>(x);
			for (pcptr c = 0; c < ColumnCount; ++c)
				for (pcptr r = 0; r < RowCount; ++r)
					Get(c, r) = (c == r) ? e : T(0);
		}
		else {
			for (pcptr idx = 0; idx < MemberCount; ++idx)
				mArray[idx] = static_cast<T>(x[idx]);
		}
	}
	...
	constexpr TME()::TMatrix(const HEAD& head, const TAIL&... tail) noexcept requires (sizeof...(TAIL) > 0) {
		...
		else LANGULUS_ASSERT("Bad element type in matrix unfolding constructor - must be ComplexNumber or Number");
	}
	...
	constexpr TME() operator * (const TME()& me, const TME()& other) noexcept {
		...
		for (pcptr i = 0; i < me.ColumnCount; ++i) {
			for (pcptr j = 0; j < me.RowCount; ++j) {
				auto& rij = result.Get(i, j);
				for (pcptr f = 0; f < me.ColumnCount; ++f)
	...
	constexpr TME() operator + (const TME()& me, const TME()& other) noexcept {
		TME() result(me);
		for (pcptr idx = 0; idx < me.MemberCount; ++idx)
	...
	/// The column vector may be standard or proxy, doesn't matter					
	///	@returns a standard row vector result											
	template<DenseNumber T, pcptr COLS, pcptr ROWS, Number K, pcptr C>
	constexpr auto operator * (const TME()& me, const TVec<K, C>& vec) noexcept requires (C <= ROWS) {
		T r[C] = {};
		for (pcptr vr = 0; vr < C; ++vr) {
			for (pcptr mc = 0; mc < pcMin(C, COLS); ++mc)
	...
	/// The row vector may be standard or proxy, doesn't matter						
	///	@returns a standard column vector result										
	template<DenseNumber T, pcptr COLS, pcptr ROWS, Number K, pcptr C>
	constexpr auto operator * (const TVec<K, C>& vec, const TME()& me) noexcept requires (C <= COLS) {
		T r[C] = {};
		for (pcptr vr = 0; vr < C; ++vr) {
			for (pcptr mc = 0; mc < pcMin(C, ROWS); ++mc)
	...
	/// The row vector may be standard or proxy, doesn't matter						
	template<DenseNumber T, pcptr COLS, pcptr ROWS, Number K, pcptr C>
	constexpr void operator *= (TVec<K, C>& vec, const TME()& me) noexcept requires (C <= COLS) {
		vec = vec * me;
	}

	template<DenseNumber T, pcptr COLS, pcptr ROWS, Number K, pcptr C>
	constexpr void operator *= (const TVec<K, C>& vec, const TME()& me) noexcept requires (C <= COLS) {
		const_cast<TVec<K, C>&>(vec) *= me;
	}
	...
	TEMPLATE()
	constexpr bool operator != (const TME()& me, const T& other) noexcept {
		return !(me == other);
	}
	...
	TEMPLATE()
	constexpr T& TME()::operator [] (const pcptr i) noexcept {
		return mArray[i];
	}
	...
	///	@return a reference to the element												
	TEMPLATE()
	constexpr const T& TME()::operator [] (const pcptr i) const noexcept {
		return mArray[i];
	}
	...
	///	@return a reference to the element												
	TEMPLATE()
	constexpr T& TME()::Get(const pcptr col, const pcptr row) noexcept {
		return mArray[col + row * ColumnCount];
	}
	...
	TEMPLATE()
	constexpr const T& TME()::Get(const pcptr col, const pcptr row) const noexcept {
		return mArray[col + row * ColumnCount];
	}

	/// Get right axis																			
	TEMPLATE()
	constexpr TVec<T, 3> TME()::GetRight() const noexcept {
		if constexpr (IsSquare && RowCount > 2)
			return { Get(0, 0), Get(1, 0), Get(2, 0) };
		else LANGULUS_ASSERT("Can't get right axis of this matrix");
	}
	
	/// Get up axis																				
	TEMPLATE()
	constexpr TVec<T, 3> TME()::GetUp() const noexcept {
		if constexpr (IsSquare && RowCount > 2)
			return { Get(0, 1), Get(1, 1), Get(2, 1) };
		else LANGULUS_ASSERT("Can't get up axis of this matrix");
	}

	/// Get view axis																				
	TEMPLATE()
	constexpr TVec<T, 3> TME()::GetView() const noexcept {
		if constexpr (IsSquare && RowCount > 2)
			return { Get(0, 2), Get(1, 2), Get(2, 2) };
		else LANGULUS_ASSERT("Can't get view axis of this matrix");
	}

	/// Get translation 																			
	TEMPLATE()
	constexpr TVec<T, 3> TME()::GetScale() const noexcept {
		if constexpr (IsSquare && RowCount > 2)
			return { GetRight().Length(), GetUp().Length(), GetView().Length() };
		else LANGULUS_ASSERT("Can't get translation of this matrix");
	}

	/// Get translation 																			
	TEMPLATE()
	constexpr TVec<T, 3> TME()::GetPosition() const noexcept {
		if constexpr (IsSquare && RowCount > 3)
	...
	/// Set translation 																			
	TEMPLATE()
	constexpr TME()& TME()::SetPosition(const TVec<T, 3>& position) noexcept {
		if constexpr (IsSquare && RowCount > 3) {
			Get(0, 3) = position[0];
			Get(1, 3) = position[1];
			Get(2, 3) = position[2];
		}
		else LANGULUS_ASSERT("Can't set translation to this matrix");
		return *this;
	}

	/// Get a whole row																			
	TEMPLATE()
	TVec<T, COLS> TME()::Row(pcptr idx) const noexcept {
		return { mArray + idx * ColumnCount };
	}

	/// Get a whole column																		
	TEMPLATE()
	TVec<T, ROWS> TME()::Column(pcptr idx) const noexcept {
		T ar[RowCount];
		for(pcptr i = 0; i < RowCount; ++i)
			ar[i] = mArray[idx + i * ColumnCount];
		return { ar };
	}
	...
	TEMPLATE()
	constexpr TME() TME()::PerspectiveFOV(
		const T& fieldOfView, const T& aspect, const T& near, const T& far) {
		static_assert(IsSquare && RowCount == 4,
	...
	/// Perspective constructor - LH perspective projection matrix					
	TEMPLATE()
	constexpr TME() TME()::PerspectiveRegion(
		const T& left, const T& right, const T& top, const T& bottom, const T& near, const T& far) {
		static_assert(IsSquare && RowCount == 4,
			"Can't make a perspective matrix from this one");
	...
	TEMPLATE()
	constexpr TME() TME()::Orthographic(
		const T& width, const T& height, const T& near, const T& far) {
		static_assert(IsSquare && RowCount == 4,
	...
	TEMPLATE()
	constexpr TME() TME()::LookAt(TVec<T, 3> forward, TVec<T, 3> up) {
		static_assert(IsSquare && RowCount > 2,
	...
	constexpr TME() TME()::Rotation(const T& roll) noexcept {
		...
		auto cosR = pcCos(roll);
		auto sinR = pcSin(roll);
	...
	TEMPLATE()
	constexpr TME() TME()::RotationAxis(const TVec<T, 3>& axis, const T& angle) noexcept {
		static_assert(IsSquare && RowCount > 2,
			"Can't make a rotation matrix from this one");

		return static_cast<ME>(TQuat<T>::FromAxisAngle(axis, angle));
	}
	
	/// Rotational constructor in euler radians (for 3x3 matrix)					
	TEMPLATE()
	constexpr TME() TME()::Rotation(
		const T& pitch, const T& yaw, const T& roll) noexcept {
		static_assert(IsSquare && RowCount > 2,
			"Can't make a rotation matrix from this one");

		const auto a = pcCos(pitch);
		const auto b = pcSin(pitch);
		const auto c = pcCos(yaw);
		const auto d = pcSin(yaw);
		const auto e = pcCos(roll);
		const auto f = pcSin(roll);
		const auto ae = a * e;
		const auto af = a * f;
		const auto be = b * e;
		const auto bf = b * f;
	...
	TEMPLATE()
	constexpr TME() TME()::Translation(const TVec<T, 4>& position) noexcept {
		ME temp = Identity();
	...
	constexpr TME() TME()::Scalar(const T& xx) noexcept {
		ME temp = Null();
		for (pcptr col = 0; col < ColumnCount; ++col)
			temp.Get(col, col) = xx;
		return temp;
	}
	...
	TEMPLATE()
	template<pcptr SIZE>
	constexpr TME() TME()::Scalar(const TVec<T, SIZE>& xx) noexcept {
		ME temp = Null();
		pcptr col;
		for (col = 0; col < pcMin(SIZE, ColumnCount); ++col)
			temp.Get(col, col) = xx[col];
		for (col = SIZE; col < ColumnCount; ++col)
	...
	TEMPLATE()
	template<pcptr SIZE, pcptr NEXT_SIZE>
	constexpr T TME()::InnerDeterminant(const T (&a)[SIZE*SIZE]) noexcept {
		if constexpr (SIZE == 0)
			...
			for (p = 0; p < SIZE; p++) {
				...
				det += a[p * SIZE] * pcPow(T(-1), T(p)) * InnerDeterminant<NEXT_SIZE>(temp);
			}
	...
	constexpr TME() TME()::Adjoint() const noexcept {
		...
		for (int i = 0; i < RowCount; i++) {
			for (int j = 0; j < RowCount; j++) {
				...
				adj.Get(i, j) = sign * temp.Determinant(RowCount - 1);
			}
		}
		return adj;
	}
	...
	TME() TME()::Invert() const {
		...
		else if constexpr (ColumnCount == 4) {
		...
		else LANGULUS_ASSERT("Matrix inversion code not implemented");
	}
